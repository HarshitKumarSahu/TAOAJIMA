...
// //     // float circle = smoothstep(0.5, 0.0, distance(uMouse, vUv));
// //     // vCircle = circle;
...
//     // Calculate distance from UV to hover point
//     // float dist = distance(uv, uHover);
//     float dist = distance(correctAspectHover, correctAspectUV);

//     // Apply Perlin noise for a subtle background effect
//     // float noise = cnoise(vec3(position.xy * 1.0, uTime * .1));

//     // Apply hover effect: displace z based on distance and hover state
//     // newPosition.z += uHoverState * 0.05 * sin(dist * 10.0 + uTime); // Reduced amplitude for smoother effect

//     // Pass noise to fragment shader for color effects
//     // vNoise = uHoverState * sin(dist * 10.0 - uTime) + noise * 0.1;

//     float scaleFactor = 0.1;

//     float waves = sin(dist * frequency - uTime * 1.125 ) * amplitude;
//     newPosition.z += waves * circle * uHoverState;

//     if(uHoverState == 1.) {
//         newPosition.x +=  -uHover.x * 50.;
//     } else {
//         newPosition.x = 0.;
//     }

//     // newPosition.x +=  -uHover.x * 50.;
//     newPosition.xy *= 1. + scaleFactor * uHoverState;
...
//     float scaleFactor = 0.2125;
//     float waves = sin(dist * frequency - uTime * 1.125) * amplitude;
//     newPosition.z += waves * circle * uHoverState;
//     // Modulate x displacement by uHoverState to reset when not hovering
